...
def calcBreakEvenEffSet():
    with open('../JSONDATA/DATA.json', 'r') as r:
        rdata = json.load(r)
    dps = list()
    for i in range (0, len(rdata)):
        dp = {}
        dp['date'] = rdata[i]['date']
        dp['BreakEvenEfficiency'] = calcBreakEvenEff(float(rdata[i]['averagehashrate'])/1000000, float(rdata[i]['averageblocktime']), float(rdata[i]['ethprice']), float(rdata[i]['timespan']), float(rdata[i]['amountOfBlocks']), getBlockReward(rdata[i]['date']), getUncleReward(rdata[i]['date']))
        dps.append(dp)
